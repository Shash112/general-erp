...
import { CreateApDocumentInput, ApDocumentLineInput, ApDocumentType } from './ap-document-model.js';
import { masterDataService } from '../../../platform/master-data/master-data.service.js';

export class ApDocumentValidator {
  public static validateContext(ctx: RequestContext, companyId: string): void {
    ...
    if (!companyId || companyId.trim() === '') {
      throw new ValidationError('AP document companyId is required.');
    }
  ...
  public static async validateSupplier(ctx: RequestContext, companyId: string, supplierId: string): Promise<void> {
    if (!supplierId || supplierId.trim() === '') {
      throw new ValidationError('Supplier ID is required for AP document.');
    }
    const supplier = await masterDataService.getSupplier(ctx, supplierId);
    if (supplier.companyId !== companyId) {
      throw new ForbiddenError(`Supplier '${supplierId}' belongs to company '${supplier.companyId}', not document company '${companyId}'.`);
    }
  }

  public static validateDocumentType(type: string): ApDocumentType {
    const validTypes: ApDocumentType[] = ['SUPPLIER_BILL', 'CREDIT_NOTE', 'DEBIT_NOTE', 'OPENING_BALANCE'];
    if (!validTypes.includes(type as ApDocumentType)) {
      throw new ValidationError(`Invalid AP documentType '${type}'. Must be one of: ${validTypes.join(', ')}.`);
    }
    return type as ApDocumentType;
  }
  ...
  public static validateLineAmountsAndScale(line: ApDocumentLineInput, seq: number): {
    ...
    if (!line.expenseAccountId || line.expenseAccountId.trim() === '') {
      throw new ValidationError(`Line ${seq} expenseAccountId is required.`);
    }
  ...
  public static async validateCreateInput(ctx: RequestContext, input: CreateApDocumentInput): Promise<void> {
    ...
    if (input.currency && input.currency !== 'INR') {
      throw new ValidationError(`Phase 2.7 AP enforces single-currency INR scope. Received currency '${input.currency}'.`);
    }
    if (input.exchangeRate && input.exchangeRate !== '1.000000') {
      throw new ValidationError(`Phase 2.7 AP enforces single-currency exchange rate '1.000000'. Received '${input.exchangeRate}'.`);
    }

    if (!input.lines || !Array.isArray(input.lines) || input.lines.length === 0) {
      throw new ValidationError('AP document must contain at least one line.');
    }

    await this.validateSupplier(ctx, input.companyId, input.supplierId);
  }
