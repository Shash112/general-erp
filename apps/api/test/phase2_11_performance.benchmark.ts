...
import { arAgingService } from '../src/modules/finance/ar-aging.service.js';
import { apAgingService } from '../src/modules/finance/ap-aging.service.js';
...
describe('Phase 2.11 — Performance Benchmark & Workload Measurement', () => {
  ...
  it('measures execution duration and performance sanity across Phase 2 reporting queries', async () => {
    ...
    const tb = await trialBalanceService.getTrialBalance(ctx, { companyId, asOfDate: '2027-03-31' });
    ...
    const gl1 = await generalLedgerReportService.getGeneralLedger(ctx, { companyId, page: 1, limit: 50 });
    ...
    const bs = await balanceSheetService.getBalanceSheet(ctx, { companyId, asOfDate: '2027-03-31' });
    ...
    const arAging = await arAgingService.getAgingSummary(ctx, companyId, '2027-03-31');
    ...
    const apAging = await apAgingService.getAgingSummary(ctx, companyId, '2027-03-31');
    ...
    const recon = await financialReportingService.getReconciliationSummary(ctx, companyId);
